...
class MasterThesisCanvas(canvas.Canvas):
    ...
    def draw_decorations(self, page_num, total_pages, main_total):
        ...
        if page_num == 2:
            ...
            self.drawString(45, 26, "Trabajo Fin de Máster — Fabian Robert Banu Stan")
            self.drawRightString(550, 26, "Índice — Pág. ii")
            self.restoreState()
            return

        # 3. Main report (Pages 3 to 22) or Appendices (Pages 23+)
        ...
        self.setFont("Helvetica", 8)
        self.setFillColor(colors.HexColor("#64748B"))
        self.drawString(45, 26, "Universidad Complutense de Madrid — Facultad de Estudios Estadísticos")
        
        if is_appendix:
            app_p = page_num - 22
            self.drawRightString(550, 26, f"Anexo — Pág. A-{app_p}")
        else:
            main_p = page_num - 2
            self.drawRightString(550, 26, f"Página {main_p} de 20")
...
def build_pdf_pipeline():
    ...
    h1_style = ParagraphStyle(
        ...
        fontName='Helvetica-Bold', fontSize=11.2, leading=14.2,
        textColor=c_primary, spaceBefore=6, spaceAfter=2.5, keepWithNext=True
    )

    h2_style = ParagraphStyle(
        'Heading2_Custom', parent=styles['Heading2'],
        fontName='Helvetica-Bold', fontSize=9.2, leading=11.8,
        textColor=colors.HexColor("#1E40AF"), spaceBefore=4, spaceAfter=1.8, keepWithNext=True
    )

    h3_style = ParagraphStyle(
        'Heading3_Custom', parent=styles['Heading3'],
        fontName='Helvetica-Bold', fontSize=8.4, leading=10.8,
        textColor=colors.HexColor("#334155"), spaceBefore=3, spaceAfter=1.2, keepWithNext=True
    )

    body_style = ParagraphStyle(
        'Body_Custom', parent=styles['Normal'],
        fontName='Helvetica', fontSize=8.0, leading=10.5,
        textColor=c_body, spaceAfter=2.5, alignment=4
    )

    bullet_style = ParagraphStyle(
        'Bullet_Custom', parent=body_style,
        leftIndent=9, firstLineIndent=-5, spaceAfter=1.5
    )

    caption_style = ParagraphStyle(
        'Caption_Custom', parent=styles['Normal'],
        fontName='Helvetica-Oblique', fontSize=7.2, leading=9.0,
        textColor=colors.HexColor("#475569"), alignment=1, spaceBefore=1.5, spaceAfter=3.0
    )

    table_cell_style = ParagraphStyle(
        'TableCell', parent=styles['Normal'],
        fontName='Helvetica', fontSize=6.8, leading=8.4, textColor=c_body
    )
    ...
    table_header_style = ParagraphStyle(
        ...
        fontName='Helvetica-Bold', fontSize=7.0, leading=8.6,
        textColor=colors.white, alignment=1
    )

    def create_callout(text, bg=c_note_bg, border=c_note_border, text_color=c_dark):
        p_style = ParagraphStyle('CalloutStyle', fontName='Helvetica', fontSize=7.2, leading=9.2, textColor=text_color)
    ...
    story.append(Spacer(1, 15))
    story.append(HRFlowable(width="90%", thickness=2, color=c_primary, spaceBefore=4, spaceAfter=14))
    ...
    story.append(HRFlowable(width="90%", thickness=1, color=c_gray_border, spaceBefore=4, spaceAfter=15))
    ...
    toc_rows = [
        ...
        [Paragraph("<b>3</b>", table_cell_bold), Paragraph("Objetivos, Alcance y Preguntas de Trabajo", table_cell_bold), Paragraph("4", table_cell_bold)],
        [Paragraph("<b>4</b>", table_cell_bold), Paragraph("Caso de Uso y Valor para Negocio Financiero", table_cell_bold), Paragraph("5", table_cell_bold)],
        [Paragraph("<b>5</b>", table_cell_bold), Paragraph("Datos y Fuentes de Información", table_cell_bold), Paragraph("6", table_cell_bold)],
        [Paragraph("<b>6</b>", table_cell_bold), Paragraph("Arquitectura de la Solución Implementada", table_cell_bold), Paragraph("8", table_cell_bold)],
        [Paragraph("<b>7</b>", table_cell_bold), Paragraph("Diseño de los Agentes y Flujo Multiagente", table_cell_bold), Paragraph("10", table_cell_bold)],
        [Paragraph("<b>8</b>", table_cell_bold), Paragraph("Metodología Analítica y Modelo de Riesgo", table_cell_bold), Paragraph("12", table_cell_bold)],
        [Paragraph("<b>9</b>", table_cell_bold), Paragraph("Implementación de la Aplicación y Productivización", table_cell_bold), Paragraph("14", table_cell_bold)],
        [Paragraph("<b>10</b>", table_cell_bold), Paragraph("Resultados y Demostración Funcional", table_cell_bold), Paragraph("16", table_cell_bold)],
    ...
    # =========================================================================
    # MEMORIA PRINCIPAL (PÁGINAS 1 A 20)
    # =========================================================================

    # -------------------------------------------------------------------------
    # 1. RESUMEN EJECUTIVO (Pág. 1)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 2. INTRODUCCIÓN Y CONTEXTO DEL PROBLEMA (Págs. 2 y 3)
    # -------------------------------------------------------------------------
    ...
    story.append(Paragraph("<b>2.1 Complejidad y Asimetría de la Información Financiera</b>", h2_style))
    story.append(Paragraph(
        "Los analistas de crédito e inversión deben reconciliar datos que operan en frecuencias temporales y formatos radicalmente distintos. Los balances auditados ofrecen rigor pero sufren de un desfase temporal significativo respecto al momento presente. Las series de precios cotizan en tiempo continuo reflejando expectativas de mercado, mientras que las noticias corporativas introducen factores imprevistos. Esta asimetría informativa dificulta la detección temprana de crisis corporativas si no se cuenta con herramientas analíticas automatizadas que integren ambas perspectivas.",
        body_style
    ))
    story.append(Paragraph("<b>2.2 Limitaciones de las Canalizaciones Monolíticas Tradicionales</b>", h2_style))
    story.append(Paragraph(
        "Los sistemas de software heredados suelen construirse como pipelines monolíticos lineales. En estas arquitecturas, el fallo en la conexión a un proveedor de noticias o una discrepancia en el formato de un balance detiene la ejecución completa del proceso, impidiendo obtener siquiera los ratios cuantitativos básicos. Asimismo, estos sistemas carecen de trazabilidad granular para medir la latencia y la tasa de error de cada transformación individual, lo que complica enormemente su mantenimiento y escalabilidad en entornos corporativos.",
        body_style
    ))
    story.append(Paragraph("<b>2.3 El Enfoque de Sistemas Multiagente (MAS)</b>", h2_style))
    story.append(Paragraph(
        "Frente a las limitaciones monolíticas, el paradigma de los Sistemas Multiagente (MAS) permite descomponer un problema analítico complejo en un conjunto de agentes cooperativos especializados. Cada agente opera de forma modular sobre una tarea acotada (extracción, análisis cuantitativo, procesamiento semántico, evaluación de reglas y generación ejecutiva). El uso de grafos de estados dirigidos (como los implementados por LangGraph) proporciona una gestión determinista del flujo, facilitando el aislamiento de excepciones y la observabilidad inmutable de cada paso.",
        body_style
    ))
    story.append(Paragraph("<b>2.4 Explicabilidad y Gobernanza en Comités de Riesgo</b>", h2_style))
    story.append(Paragraph(
        "En los comités de riesgo y admisión crediticia, los modelos de 'caja negra' carecen de validez operativa. Cualquier sistema analítico debe ser capaz de justificar detalladamente por qué una empresa se clasifica en un determinado nivel de riesgo. El presente proyecto adopta un modelo híbrido donde las alertas de riesgo se desglosan en valores numéricos observados, umbrales explícitos y justificaciones financieras comprensibles para directivos y analistas de negocio.",
        body_style
    ))
    story.append(Paragraph("<b>2.5 Mapa de Integración Curricular con el Máster UCM</b>", h2_style))
    story.append(Paragraph(
        "El desarrollo del proyecto sintetiza de forma práctica y transversal las materias impartidas en el plan de estudios del Máster en Big Data, Data Science & Inteligencia Artificial:",
        body_style
    ))
    curr_data = [
        [Paragraph("<b>Módulo Docente del Máster</b>", table_header_style), Paragraph("<b>Técnica / Tecnología Aplicada</b>", table_header_style), Paragraph("<b>Componente en el Repositorio</b>", table_header_style)],
        [Paragraph("Programación Python", table_cell_bold), Paragraph("Tipado estricto Pydantic v2, POO, Pandas y NumPy.", table_cell_style), Paragraph("<code>app/domain/models.py</code>", table_cell_style)],
        [Paragraph("Bases de Datos SQL", table_cell_bold), Paragraph("Esquema relacional (8 tablas), ORM SQLAlchemy, SQLite.", table_cell_style), Paragraph("<code>app/persistence/database.py</code>", table_cell_style)],
        [Paragraph("Estadística y Machine Learning", table_cell_bold), Paragraph("Volatilidad anualizada, Isolation Forest (scikit-learn).", table_cell_style), Paragraph("<code>app/analytics/anomaly_detection.py</code>", table_cell_style)],
        [Paragraph("IA y Procesamiento NLP", table_cell_bold), Paragraph("Orquestación LangGraph, FAISS vector store, OpenAI LLM.", table_cell_style), Paragraph("<code>app/orchestration/graph.py</code>", table_cell_style)],
        [Paragraph("Productivización de Modelos", table_cell_bold), Paragraph("API REST asíncrona FastAPI, BackgroundTasks, Uvicorn.", table_cell_style), Paragraph("<code>app/api/main.py</code>", table_cell_style)],
        [Paragraph("Visualización Avanzada", table_cell_bold), Paragraph("Cuadro de mando interactivo en Streamlit, Matplotlib.", table_cell_style), Paragraph("<code>app/ui/streamlit_app.py</code>", table_cell_style)],
    ]
    ...
    # -------------------------------------------------------------------------
    # 3. OBJETIVOS, ALCANCE Y PREGUNTAS DE TRABAJO (Pág. 4)
    # -------------------------------------------------------------------------
    ...
    story.append(Paragraph("<b>Preguntas de Trabajo e Hipótesis de Investigación:</b>", h3_style))
    story.append(Paragraph("• <b>PQ1 (Modularidad y Trazabilidad):</b> ¿Permite la arquitectura multiagente reducir la fragilidad frente a caídas de servicios y registrar métricas de observabilidad detalladas? <i>Hipótesis validada: El wrapper <code>wrap_agent</code> registra inmutablemente el estado y duración en ms de cada agente.</i>", bullet_style))
    story.append(Paragraph("• <b>PQ2 (Hibridación Analítica):</b> ¿Mejora la combinación de reglas deterministas y Machine Learning la capacidad de detección de riesgo? <i>Hipótesis validada: Las reglas capturan solvencia estructural y el Isolation Forest turbulencias bursátiles recientes.</i>", bullet_style))
    story.append(Paragraph("• <b>PQ3 (Control de Alucinaciones):</b> ¿Garantizan los esquemas Pydantic forzados la veracidad factual en la generación de informes con LLM? <i>Hipótesis validada: El esquema <code>ExecutiveReport</code> restringe las salidas a datos observados.</i>", bullet_style))
    story.append(PageBreak())

    # -------------------------------------------------------------------------
    # 4. CASO DE USO Y VALOR PARA NEGOCIO FINANCIERO (Pág. 5)
    # -------------------------------------------------------------------------
    ...
    story.append(Paragraph("<b>Perfiles de Usuario Beneficiarios:</b>", h3_style))
    story.append(Paragraph("• <b>Analista de Riesgo de Crédito Bancario:</b> Utiliza el sistema para realizar un triaje preliminar de solicitudes de financiación corporativa, identificando desequilibrios patrimoniales, coberturas deficitarias o apalancamiento excesivo en segundos.", bullet_style))
    story.append(Paragraph("• <b>Gestor de Carteras de Renta Variable:</b> Monitorea emisores en cartera en busca de divergencias entre la estabilidad contable reportada y anomalías estadísticas de negociación en el mercado bursátil.", bullet_style))
    story.append(Paragraph("• <b>Comité de Admisión y Cumplimiento:</b> Dispone de un informe ejecutivo estructurado y auditable en formato JSON, con justificación clara de cada señal para la toma colegiada de decisiones.", bullet_style))

    story.append(Paragraph("<b>Matriz de Capacidades y Valor de Negocio:</b>", h3_style))
    bus_rows = [
    ...
    # -------------------------------------------------------------------------
    # 5. DATOS Y FUENTES DE INFORMACIÓN (Págs. 6 y 7)
    # -------------------------------------------------------------------------
    ...
    story.append(Paragraph(
        "<b>Generación Sintética Controlada de Perfiles (scripts/generate_fixtures.py):</b> Para validar el comportamiento del sistema ante condiciones extremas de estrés sin depender de la volatilidad o lagunas de APIs externas, se diseñó un generador determinista que modeliza tres perfiles corporativos con 3 ejercicios contables completos (2022, 2023, 2024), 252 sesiones bursátiles anuales simuladas mediante paseos aleatorios gaussianos con deriva controlada y noticias contextuales acordes al perfil.",
        body_style
    ))
    ...
    # -------------------------------------------------------------------------
    # 7. DISEÑO DE LOS AGENTES Y FLUJO MULTIAGENTE (Págs. 10 y 11)
    # -------------------------------------------------------------------------
    ...
    story.append(Paragraph("<b>Fichas Técnicas de los Agentes Implementados:</b>", h3_style))
    agent_rows = [
    ...
    story.append(Paragraph(
        "<b>Mecanismo de Envoltorio y Trazabilidad (wrap_agent):</b> Cada agente se encapsula en una función de orden superior que intercepta la ejecución, cronometra el tiempo exacto en milisegundos y genera un registro <code>AgentEvent</code> inmutable que se almacena en el estado y se persiste en SQLite, asegurando que ninguna acción quede sin auditar.",
        body_style
    ))
    story.append(PageBreak())

    # -------------------------------------------------------------------------
    # 8. METODOLOGÍA ANALÍTICA Y MODELO DE RIESGO (Págs. 12, 13, 14)
    # -------------------------------------------------------------------------
    ...
    story.append(Paragraph("<b>Formulación Matemática de Indicadores Principales:</b>", h3_style))
    story.append(Paragraph("• <b>Margen EBITDA:</b> $\\text{EBITDA} / \\text{Revenue}$ &nbsp;|&nbsp; <b>Margen Neto:</b> $\\text{Net Income} / \\text{Revenue}$", body_style))
    story.append(Paragraph("• <b>Deuda Neta / EBITDA:</b> $(\\text{Total Debt} - \\text{Cash}) / \\text{EBITDA}$ &nbsp;|&nbsp; <b>Ratio Corriente:</b> $\\text{Current Assets} / \\text{Current Liabilities}$", body_style))
    story.append(Paragraph("• <b>Caída Máxima (Max Drawdown):</b> $\\max_{t} [(\\max_{s \\le t} P_s - P_t) / \\max_{s \\le t} P_s]$ &nbsp;|&nbsp; <b>Volatilidad Anualizada:</b> $\\sigma_{\\text{diaria}} \\times \\sqrt{252}$", body_style))
    ...
    # -------------------------------------------------------------------------
    # 9. IMPLEMENTACIÓN DE LA APLICACIÓN Y PRODUCTIVIZACIÓN (Págs. 14 y 15)
    # -------------------------------------------------------------------------
    ...
    story.append(Paragraph(
        "<b>Endpoints REST y Ciclo de Vida Asíncrono:</b> La API desacopla la recepción de peticiones del cómputo intensivo. Al recibir una solicitud en <code>POST /analyses</code>, el servidor registra la entidad <code>AnalysisRun</code> en estado <code>pending</code> y despacha la ejecución del grafo a <code>BackgroundTasks</code>. El cliente (Streamlit o cualquier servicio corporativo) realiza un sondeo no bloqueante mediante <code>GET /analyses/{run_id}</code> hasta que el estado transita a <code>completed</code>.",
        body_style
    ))
    ...
    # -------------------------------------------------------------------------
    # 10. RESULTADOS Y DEMOSTRACIÓN FUNCIONAL (Págs. 16 y 17)
    # -------------------------------------------------------------------------
    ...
    for b in bibs:
        story.append(Paragraph(b, ParagraphStyle('Bib', fontName='Helvetica', fontSize=7.2, leading=9.4, spaceAfter=2)))
    ...
    print(f"Master Thesis PDF generated: {filename}")
